...
def gm_two_seasons_annual_mads(ds):
    ...
    gm = dc.load(product='ga_s2_gm', like=ds) #should load the same spatial extent,crs etc.
    
    def annual_tmad_fun(ds, gm):
        tmad = xr_tmad(ds, gm) # write separate tmad function that calls on kirill's geomedian
        tmad['sdev'] = -np.log(tmad['sdev'])
        tmad['bcdev'] = -np.log(tmad['bcdev'])
        tmad['edev'] = -np.log(tmad['edev'])
        
    annual_tmad = annual_tmad_fun(ds, gm)
    # this is an xarray holding only the annual tmad
...
#         gm_mads['bcdev'] = -np.log(gm_mads['bcdev'])
#         gm_mads['edev'] = -np.log(gm_mads['edev'])
# these should happen in the annual_tmad_fun() function
...
def xr_tmad(ds, gm, axis='time', where=None, **kw):
    ...
    **kwargs -- passed on to pcm.gnmpcm
    ...
    def tmad(arr, gm, **kw):
        ...
        smad = hdstats.smad_pcm(arr, gm, num_threads=nt)[:,:, np.newaxis]
        bcmad = hdstats.bcmad_pcm(arr, gm, num_threads=nt)[:,:, np.newaxis]
        return np.concatenate([emad, smad, bcmad], axis=-1)
    ...
    if is_dask:
        ...
        data = da.map_blocks(lambda x: tmad(x, **kw),
                             xx_data,
                             name=randomize('geomedian'),
                             dtype=xx_data.dtype, 
                             chunks=xx_data.chunks[:-2] + (xx_data.chunks[-2][0]+3,),
